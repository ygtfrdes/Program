...
plot_amayes <- function(gnet, 
  ...
  plot(gnet, edge.color=edge_colors, vertex.color=node_colors, 
       edge.label=NA, vertex.label=NA, vertex.size=norm(node_data), edge.size=norm(node_data), main=main)
  ...
  legend("bottomright", 
         ...
         text.col= node_unique_color, 
         col= node_unique_color, 
         bty="n", pch=20 , cex=1, pt.cex=node_unique_data,
         title = node_titre, title.col ="black")
  ...
  y <-  a$text$y
  symbols(x, y, circles=norm2(node_unique_data), inches=FALSE, add=TRUE, bg=node_unique_color)
}
norm  <- function(data){(data - min(data)) / (max(data) - min(data)) * 20}
norm2 <- function(data){(data - min(data)) / (max(data) - min(data)) * .11}


plot_edge <- function(gnet, 
                        node_data = c(rep(0, vcount(gnet))), 
                        edge_data = c(rep(0, ecount(gnet))), 
                        node_titre = "",
                        edge_titre = "",
...
plot_all <- function(gnet,data, edge_data, titre, edge_titre, main, ij = i) {
  
  edge_unique_data <- sort(unique(edge_data))
  edge_color_function <- colorRampPalette(c("white", "dark red")) 
  edge_unique_color <- sort(edge_color_function(length(edge_unique_data)), decreasing = T)
  edge_colors <- edge_unique_color[match(edge_data, edge_unique_data)]
  ...
  plot(gnet, edge.color=edge_colors, vertex.color=colors, edge.label=NA, vertex.label=NA, vertex.size=3, edge.size=6, main=main)
  legend("bottomright", 
         legend= unique_data, 
         text.col= unique_color, 
         col= unique_color, 
