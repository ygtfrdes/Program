...
for i = 1:preamble_len
    [out_preamble] = LoRa_Modulation(SF_t,BW,Fs,num_samples,0,inverse);
...
inverse = 1;
for i = 1:sync_len
    [out_sync] = LoRa_Modulation(SF_t,BW,Fs,num_samples,32,inverse);
    outp = [outp out_sync];
end
...
inverse = 0;
for i = 1:total_sym
    [out_sym] = LoRa_Modulation(SF_t,BW,Fs,num_samples,symbols(i),inverse);
    outp = [outp out_sym];
end
...
[out_reverse] = LoRa_Modulation(SF_e,BW,Fs,num_samples,0,inverse);
