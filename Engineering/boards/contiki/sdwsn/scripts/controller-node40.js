...
function flowEntryInstall(installnode,flowid,ipv6src,ipv6dst,nxhop) {
	var req = coap.request({host: installnode , pathname: '/sdwsn/flow-mod', method: 'PUT' , query: 'action=insert&flowid=' + flowid + '&ipv6src="' + ipv6src + '"&ipv6dst="' + ipv6dst + '"&nhmacaddr="' +  nxhop +'"&txpwr="3"', retrySend: 5  })
