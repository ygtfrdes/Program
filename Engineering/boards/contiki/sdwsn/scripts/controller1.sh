...
#while [ 1 ]; do
  for ((i=2;i<=$num_motes-1;i++)); do
    ihex=`echo "ibase=10;obase=16;$i"|bc` 
    routes=`$coap_client -B 3 -m get coap://[fd00::200:0:0:$ihex]:5683/sdwsn/routes | grep -v CON`
    let routes++
    echo "mote=$ihex"
    echo $routes
      for ((j=1;j<=$routes;j++)); do
         $coap_client -B 3 -m put coap://[fd00::200:0:0:$ihex]:5683/sdwsn/flow-mod/insert?i=2,id=2001:0DB8:10::20,n=F978
      done
  done
#  sleep 1200;
#done
