	...
	if (my_index == -1) {
		...
		if (my_position > 0) {
			...
			for (i = 0; i < n_windows; ++i) {
				add_window(e,
									 get_window_from_array(&(p->payload[i * WINDOW_SIZE + 1])));
			}

			action_t *a =
					create_action(FORWARD_U, &(p->payload[prev]), ADDRESS_LENGTH);
			add_action(e, a);
			add_entry(e);
		}
		...
		if (my_position < path_len - 1) {
			...
			action_t *a =
					create_action(FORWARD_U, &(p->payload[next]), ADDRESS_LENGTH);
			add_action(e, a);
			add_entry(e);
	...
	if (is_my_address(&(p->header.dst))) {
		...
		if (!is_my_address(&(p->header.src))) {
			...
			if ((p->payload[i] & 128) == CNF_READ) {
				...
				case GET_RULE:
					p->header.len +=
							get_array_from_entry_id(&p->payload[i + 2], p->payload[i + 1]);
					break;
			...
			print_packet_uart(p);
				...
				case RULE_TTL:
					if (conf_size[id] == 1) {
						memcpy((uint8_t *)conf_ptr[id], &(p->payload[i + 1]),
									 conf_size[id]);
					} else if (conf_size[id] == 2) {
